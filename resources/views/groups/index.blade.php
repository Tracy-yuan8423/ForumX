        ...
        <div class="fs-groups">
            @if (fs_db_config('menu_group_type') == 'tree')
                ...
                @foreach($groupTree ?? [] as $tree)
                    <div class="card rounded-0 mb-4 @if (fs_db_config('fs_forumx_group_index') < 2) pb-4 @endif">
                        <div class="card-header mb-4 fw-bolder" style="background-color: #E5EDF2;color: #369;">{{ $tree['gname'] }}</div>
                        @if (fs_db_config('fs_forumx_group_index') > 1)
                            @php
                                $colNumber = 6;
                                if (fs_db_config('fs_forumx_group_index') == 3) {
                                    $colNumber = 4;
                                }
                            @endphp
                            <div class="row">
