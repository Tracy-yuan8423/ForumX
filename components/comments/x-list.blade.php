...
<div class="row mx-0" id="{{ $comment['cid'] }}">
    <div class="col-md-2 px-0 fs-text-decoration" style="background: #E5EDF2;border-bottom: 4px solid #C2D5E3;">
        ...
        <div class="ps-3 py-1 fw-semibold" style="border-bottom: 1px dashed #CDCDCD;">
            ...
            @else
                {{-- 正常作者 --}}
                <a href="{{ route('fresns.profile.index', ['uidOrUsername' => $comment['author']['uid']]) }}" target="_blank" class="link-dark fs-7">{{ $comment['author']['nickname'] }}</a>
        ...
        {{-- 用户资料 --}}
        <div class="px-3">
            @component('components.comments.sections.x-author', [
                'cid' => $comment['cid'],
