...
<tr>
    <td class="w-75 ps-lg-3">
        ...
        <a href="{{ route('fresns.post.detail', ['pid' => $post['pid']]) }}">{{ $post['title'] ?? Str::limit(strip_tags($post['content']), 40) }}</a>
    ...
    <td class="dropdown fs-content-author">
        ...
        @else
            {{-- 正常作者 --}}
            <a href="{{ route('fresns.profile.index', ['uidOrUsername' => $post['author']['uid']]) }}">{{ $post['author']['nickname'] }}</a>
        @endif
    ...
    <td>
        @if ($post['commentCount'])
            <span class="badge text-bg-light fw-normal">{{ $post['commentCount'] }}</span>
        @endif
    </td>
    @desktop
        <td class="pe-lg-3 fs-8 text-secondary" style="padding-top: 12px;">{{ $post['lastCommentTimeAgo'] }}</td>
    @enddesktop
</tr>
